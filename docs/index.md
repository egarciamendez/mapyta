...
👉 **[See the Showcase](showcase.md)** — every feature as a live, interactive map on a single page.
...
- 🗺️ **[Shapely-native](tutorial/geometries.md)** — pass `Point`, `Polygon`, `LineString` directly, including multi-geometries
- 📝 **[Markdown/HTML hover and popup text](tutorial/tooltips-popups.md)** — bold, italic, links, lists, and code in tooltips
- 📌 **[Emoji & icon markers](tutorial/markers.md)** — any text or Font Awesome icons as markers
- 🎨 **[Choropleth & heatmaps](tutorial/choropleth.md)** — color-coded maps from numeric data
- 📊 **[GeoPandas integration](tutorial/geodataframe.md)** — `Map.from_geodataframe()` one-liner
- 🗂️ **[DataFrame support](tutorial/dataframe.md)** — `add_dataframe()` works with any Pandas or Polars DataFrame with a WKT geometry column
- 🌐 **[Auto CRS detection](tutorial/coordinates.md)** — RD New (EPSG:28992) coordinates transform automatically
- 📤 **[Export anywhere](tutorial/export.md)** — HTML, PNG, SVG, GeoJSON, async variants, and `BytesIO` buffers
- 🧩 **[Feature groups](tutorial/layers.md)** — toggleable layers with a built-in layer control
- 🔌 **[Set of tile providers](tutorial/configuration.md)** — OpenStreetMap, CartoDB, Esri, Stamen, and Kadaster
